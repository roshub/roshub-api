...
const debug = require('debug')('roshub.comms.ble-peer')
...
class BLEPeerClient extends EventEmitter {
  ...
  static get ROSHUB_SERVICE_UUID(){ return 0x11311 }
  ...
  async start(autoreconnect = true){
    ...
    try {
      ...
      const primaryService = await this.bleDevice.gatt.getPrimaryService(BLEPeerClient.ROSHUB_SERVICE_UUID)
  ...
  static requestDevice(shortId){
    ...
    if (!shortId){
      ...
      filters.push({
        services: [BLEPeerClient.ROSHUB_SERVICE_UUID],
        namePrefix: 'RosHub'
      })
